...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const totalX = (x2 - x1) ** 2;
  const totalY = (y2 - y1) ** 2;
  const distance = Math.sqrt(totalX + totalY);
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAngle = dot / (vectorOne * vectorTwo);
...
function roundToPowerOfTen(num, pow) {
  const digit = Math.round(num / 10 ** pow);
  return digit * 10 ** pow;
}
